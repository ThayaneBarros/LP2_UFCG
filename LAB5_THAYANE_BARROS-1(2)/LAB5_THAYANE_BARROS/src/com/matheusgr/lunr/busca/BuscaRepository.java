...
class BuscaRepository {
	 ...
	 * 
	 * @param tipoDeBusca O tipo de busca que realizará a operação de busca realizada.
	 * @param documentos Documentos resultantes dessa busca.
	 */
	public void adicionaBusca(BuscaInterface tipoDeBusca, DocumentoDTO[] documentos) {
		String[] ids = Stream.of(documentos)
			.map(DocumentoDTO::getId)
			.collect(Collectors.toList())
			.toArray(new String[] {});
		this.buscas.add(new HistoricoBusca(tipoDeBusca, ids));
	}
