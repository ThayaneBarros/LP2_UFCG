...
class ValidadorBusca extends ValidadorPadrao {
	...
	/**
	 * Valida os metadados da busca. Os metadados não podem ser objetos nulos ou vazios.
	 * 
	 * @param metadados Os metadados a serem buscados.
	 */
	public void validaMetadados(Map<String, String> metadados) {
		Objects.requireNonNull(metadados, "Conteúdo não pode ser nulo");
		for (String meta : metadados.keySet()) {
			if (meta.isBlank() || metadados.get(meta).isBlank()) {
				throw new IllegalArgumentException("O metadado não deve possuir chaver ou valor vazio");
			}
		}
		return;
	}
